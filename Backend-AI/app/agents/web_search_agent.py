...
class WebSearchAgent:
    ...
    async def process(
        ...
        try:
            ...
            # System prompt
            system_prompt = f"""Ты - эксперт по поиску информации в интернете для владельцев домашних животных.
...
Шаг 2: Выбери 2-3 САМЫХ РЕЛЕВАНТНЫХ результата и вызови fetch_webpage для них
Шаг 3: Сформируй итоговый ответ с полным текстом выбранных страниц

**Формат итогового ответа:**
Возвращай JSON с полями query (поисковый запрос) и resources (массив объектов).
Каждый объект в resources должен содержать: title (название), url (ссылка), summary (текст страницы).
Для страниц с полным текстом: summary = полный текст из fetch_webpage
Для остальных страниц: summary = краткий сниппет из search_web

**ВАЖНО:**
- Загружай полный контент только для 2-3 САМЫХ РЕЛЕВАНТНЫХ страниц
- Для остальных используй краткие сниппеты из search_web
- Оркестратор сам решит что делать с полным текстом
...
**Итоговый результат:**
- Возвращай структурированный JSON с полным текстом для 2-3 самых релевантных страниц
- Для остальных страниц используй краткие сниппеты
- Оркестратор решит: показать пользователю или проиндексировать в RAG
- При медицинских вопросах - всегда рекомендуй консультацию ветеринара"""
