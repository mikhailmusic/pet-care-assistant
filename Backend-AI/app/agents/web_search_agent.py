...
async def fetch_webpage(url: str, max_length: int = 5000) -> str:
    ...
    Args:
        ...
        max_length: Максимальная длина текста (по умолчанию 5000 символов)
...
2. **fetch_webpage(url, max_length)** - Загрузка полной страницы
   - Когда пользователь дал ссылку
   - Когда нужно прочитать найденную статью целиком
...
**Важно:**
- Твой результат получит ОРКЕСТРАТОР (не пользователь напрямую)
- Возвращай структурированный JSON
- Оркестратор решит: показать пользователю или проиндексировать
- При медицинских вопросах - всегда рекомендуй консультацию ветеринара

Формулируй запросы точно и эффективно!"""
